function Paralle_Create_Pairwise_Image_Augmentation(Par_Index,SUNRGBDMeta,Image_Path,Train_Val,Test,trainvalOut,testOut,List,resizeRowCol,Max,Augmentation_Num,move_range)
% test
...
% par out names
trainvalOut=strcat(trainvalOut,'_',int2str(Par_Index),'.txt');
testOut=strcat(testOut,'_',int2str(Par_Index),'.txt');
...
startIndex=1549+1000*(Par_Index-1);
endIndex=1549+1000*(Par_Index)-1;
    ...
    for j=1:size(Valid_Obj_List,2)
        for k=1:size(Valid_Obj_List,2)
            if j~=k
                ...
                if size(find(Train_Val==imageid),2)>0 && TrainValPerClass_Current(pairwise_index)<Max
                    ...
                    if times>0
                        ...
                        for time=1:times
                            ...
                            imagepath=strcat(int2str(Par_Index),'_',zeroAhead,int2str(Image_Index),'.jpg');
                ...
                % this is for testing
                elseif TestPerClass_Current(pairwise_index)<Max
