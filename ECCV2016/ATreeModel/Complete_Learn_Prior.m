...
% index of training 
Training_Image_Index=1:10335;
Num_training=size(Training_Image_Index,2);
Annotation_Path='/Users/ruichiyu/Desktop/Research/Winter/Annotation_Complete1_All/';
...
for i=1:size(Prior_Co_Occur,1)
    for j=1:size(Prior_Co_Occur_Conditional_Prob,1)
        % add-k smoothing
        Prior_Co_Occur_Conditional_Prob(i,j)=(Prior_Co_Occur_Conditional_Prob(i,j)+1/size(Prior_Co_Occur_Conditional_Prob,1))/(Prior_Object_Presence(j)+1);
    end
end
...
%% store all priors
save('Complete_Prior_39_New.mat','Prior_Co_Occur_Conditional_Prob','Prior_Scale_Histogram','Prior_Spatial_Histogram_X','Prior_Spatial_Histogram_Y','Prior_Object_Presence','Count_Object_Pair')
