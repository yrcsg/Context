function [rec,prec,ap] = SUNevaldet(id,cls,draw)
%% Define paths
annoPath = '~/Dropbox/Context/mAP/Annotation_Complete1/';
detresFolder = '~/Dropbox/Context/mAP/';
detresPath = [detresFolder '%s_det_val_%s.txt'];
minoverlap = 0.5000;
%%
% load test set
...
for i=1:length(gtids)
    ...
    % read annotation
    rec=SUNreadrecord(sprintf('%s%s.txt',annoPath,gtids{i}));
...
% load results
resfile = fopen(sprintf(detresPath,id,cls));
